    ...
    public function index(Request $request)
    {
        if (empty($request->lokasi)) {
            $data2 = DB::table('orders')
                ...
                ->select('orders.id', 'orders.table_number', 'orders.discount AS bayar', 'orders.disc AS discount',
                    'order_details.product_price', 'order_details.quantity', 'orders.total', 'orders.keperluan'
